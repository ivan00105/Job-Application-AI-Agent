...
4. **Write database queries** using the async PostgreSQL client in `database/postgres_client.py`
...
1. **Create migration file** in `backend/migrations/` with timestamp prefix: `YYYYMMDDHHMMSS_description.sql`
...
5. **Apply migration** using the provided scripts (see `backend/scripts/run_migration.py`) or your preferred SQL client
...
│
├── backend/migrations/  # Database migrations (add schema changes here)
